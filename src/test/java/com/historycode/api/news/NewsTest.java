...
public class NewsTest extends ApiTestRunner {
    ...
    @Step("Deleting a new image after a request")
    private void deleteNewImg() {
        Response response = imageClient.delete(imageId);
        Assert.assertEquals(response.getStatusCode(), 200);
    }
    ...
    private int createNewImg() {
        ...
        String title = "TestImg" + System.currentTimeMillis();

        newsImage.setTitle(title);
        newsImage.setBaseFormat(pngImg);
        newsImage.setMimeType("image/png");
        newsImage.setExtension("png");
        newsImage.setAlt("1");

        Assert.assertEquals(imageClient.post(newsImage).getStatusCode(), 200);
        Response response = imageClient.getAll();
        Assert.assertEquals(response.getStatusCode(), 200);
        List<ImageApi> images = response.jsonPath().getList("", ImageApi.class);

        return images.stream()
                .filter(img -> img.getImageDetails() != null && title.equals(img.getImageDetails().getTitle()))
                .map(image -> image.getImageDetails().getId())
                .findFirst()
                .orElse(0);
    }
}
